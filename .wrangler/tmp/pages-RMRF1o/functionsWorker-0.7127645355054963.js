...
${content}`;
    ...
    return new Response(JSON.stringify({ error: "AI bullets failed", details: String(err?.message || err) }), { status: 500, headers: { "Content-Type": "application/json" } });
  }
}, "onRequestPost");
...
${content}`;
    ...
    return new Response(JSON.stringify({ error: "AI rewrite failed", details: String(err?.message || err) }), { status: 500, headers: { "Content-Type": "application/json" } });
  }
}, "onRequestPost");
...
${content}`;
    ...
    return new Response(JSON.stringify({ error: "AI summarize failed", details: String(err?.message || err) }), { status: 500, headers: { "Content-Type": "application/json" } });
  }
}, "onRequestPost");
...
// ../.wrangler/tmp/pages-RMRF1o/functionsRoutes-0.17743832016411498.mjs
var routes = [
  {
